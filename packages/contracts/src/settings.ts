...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    ...
    launchArgs: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "e.g. --add-dir /srv/shared",
          clearWhenEmpty: "omit",
        },
      }),
    ),
    showRateLimits: Schema.Boolean.pipe(
      ...
      Schema.annotateKey({
        title: "Show usage limits",
        description: "Show remaining Antigravity limits beside the composer controls.",
        providerSettingsForm: { control: "switch", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
  ...
  {
    order: ["binaryPath", "launchArgs", "showRateLimits"],
  },
);
