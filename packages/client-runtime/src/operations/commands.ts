...
export type PortableImportThreadInput = CommandInput<"thread.portable.import">;
...
export const importPortableThread: (input: PortableImportThreadInput) => CommandEffect = Effect.fn(
  "EnvironmentCommands.importPortableThread",
)(function* (input) {
  const metadata = yield* timestampedCommandMetadata(input);
  return yield* dispatch({
    ...input,
