...
function mapGoUsageWindow(
  ...
  const usedPercent = Math.min(100, Math.max(0, window.percent));
  const resetsAt =
    typeof window.resetsAt === "string" ? DateTime.make(window.resetsAt) : Option.none();
  return {
    id,
    kind,
    label,
...
export function normalizeOpenCodeGoUsage(
  ...
  const limits = [
    mapGoUsageWindow("rolling", "session", "Session", windows.rolling, FIVE_HOUR_WINDOW_MINUTES),
    mapGoUsageWindow("weekly", "weekly", "Weekly", windows.weekly, WEEKLY_WINDOW_MINUTES),
    mapGoUsageWindow("monthly", "monthly", "Monthly", windows.monthly, MONTHLY_WINDOW_MINUTES),
...
export const loadOpenCodeGoUsage = Effect.fn("OpenCodeProvider.loadOpenCodeGoUsage")(function* (
  ...
  return normalizeOpenCodeGoUsage(payload, checkedAt);
});
