// This file mostly exists because we want dev mode to say "T3 Science (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "T3 Science (Dev)" : "T3 Science (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.t3tools.t3science.dev.${devBundleIdSuffix || "local"}`
  : "com.t3tools.t3science";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["t3science-dev"] : ["t3science"];
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the T3 Science name.
