...
Local desktop development uses `t3science-dev://app`, while packaged builds use `t3science://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `t3science-dev://app`; the production Clerk instance should only need
`t3science://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["t3science://app"]}'
```
...
The production macOS bundle ID is `com.t3tools.t3science`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.t3tools.t3science` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.t3tools.t3science`.
...
T3CODE_PORT=13773 \
  "/Applications/T3 Science (Alpha).app/Contents/MacOS/T3 Science (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/T3 Science (Alpha).app"
codesign -d --entitlements :- "/Applications/T3 Science (Alpha).app"
```
